...
ld_prune <- function(df_snps,
  ...
  for (i in chr){
    ...
    cur_snps <- cur_chr %>%
      pull(ChrPos)
    print(paste0('Only one SNP! cur_snps: ', cur_snps))
  ...
  fwrite(
    x = clipped_res,
    file = here::here(output_dir, sprintf("snpClip_results_%s_chr%i.txt", pop, i)),
    quote = FALSE,
    sep = "\t"
  )
...
find_variants_needing_proxies <- function(gwas_variant_df, var_nonmissingness,
  ...
  tibble(VAR_ID=need_proxies_varid) %>%
    ...
    left_join(gwas_variant_df[,c("VAR_ID","P_VALUE")], by="VAR_ID")
}
